<template>
  ...
  <br />
  Description:<br />
  <div class="text-h5">{{ row.description }}</div>
  <br />
  Location:<br />
  <strong>{{ row.storage_location }}</strong> -> {{ row.stock }}
  {{ row.stock_unit }}<br />
  Condition: {{ row.condition }}<br />
  Status: {{ row.state }}<br />
  <div class="q-pa-md" style="max-width: 1000px">
    <form @submit.prevent.stop="onSubmit" class="q-gutter-y-md">
      <q-input filled dense type="number" label="Quantity" />
      <q-input filled dense type="text" label="Comment" />
      <q-btn
        label="Update"
        type="submit"
        color="primary"
  ...
  <q-img
    v-if="row.image !== null && !row.image.endsWith == 'None'"
    :src="row.image"
    style="height: 100px; width: 100px"
  />

  <q-input filled readonly type="textarea" />
  ...
  <div
    v-if="
      row.alternative_locations != Null && row.alternative_locations.length > 0
    "
  >
    <br />
    <q-separator />
    ...
    >
      <template v-slot:top-right>
        <q-btn
          ...
          @click="onDistributorDataRequest(row.distributor_pk_set_urlencoded)"
        />
      </template>
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td v-for="col in props.cols" :key="col.name" :props="props">
            ...
            <a :href="col.value.url" v-if="(col.name = 'distributor')">{{
              col.value.name
            }}</a>
            <a
              :href="col.value.url"
  ...
  <q-separator />
  <div class="text-left">
    Inventory ID: {{ row.id }}, Part ID: {{ row.part_pk }}
  </div>
</template>
